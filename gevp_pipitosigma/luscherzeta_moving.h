...
public:
  ...
  inline GSLvector operator+ (const GSLvector &r) const
  {
    GSLvector out(*this);
    out += r;
    return out;
...
class LuscherZeta{
  ...
  GSLvector d; 		//CM-momentum, in unit of 2pi/L 
  GSLvector dnorm; 	//Normalised CM-momentum vector
  ...
  double z3sum(const double q, const GSLvector &n, const double gamma) const
  {
//    printf("z3sum with q = %f and n = %f %f %f\n",q,n[0],n[1],n[2]);
//    printf("z3sum with q = %f and d = %f %f %f\n",q,d[0],d[1],d[2]);
//    printf("z3sum with q = %f and l = %f %f %f\n",q,l[0],l[1],l[2]);
//    GSLvector tmp(n);
//    GSLvector tmpd(d);
//    printf("z3sum with q = %f and tmpd(d) = %f %f %f\n",q,tmpd[0],tmpd[1],tmpd[2]);
//    tmpd += l;
//    printf("z3sum with q = %f and tmpd(d+l) = %f %f %f\n",q,tmpd[0],tmpd[1],tmpd[2]);
//    tmpd *= 0.5;
//    printf("z3sum with q = %f and tmpd = %f %f %f\n",q,tmpd[0],tmpd[1],tmpd[2]);
//    tmp += tmpd;
//    GSLvector r_raw = tmp;
    GSLvector r_raw = n + l*0.5 + d*0.5;
    double r_pall_raw = dot(r_raw,dnorm);
    GSLvector rp = r_raw - r_pall_raw * dnorm;
    double r_parr = r_pall_raw / gamma;
    double r2 = square(r_parr) + rp.norm2();

    double q2 = square(q);
    double out = 0.0;
//    printf("z3sum with q = %f and r_raw = %f %f %f\n",q,r_raw[0],r_raw[1],r_raw[2]);    
    ...
    double gn2 = square(dcoeff*gamma) + np.norm2();

    double int_n = int_zeta(q*q,gn2);
//tmp = d;
//tmp +=l;
    out += int_n*pow(-1, dot(n,(d+l)))*gamma;

    return out;
  }
  
 public:
  LuscherZeta(): l(3), d(3), dnorm(3), N(5), epsabs(1e-06),epsrel(1e-06) {}
  LuscherZeta(const double x, const double y, const double z, const double px, const double py, const double pz): l(3), d(3), dnorm(3), N(5), epsabs(1e-06),epsrel(1e-06){
    setTwists(x,y,z);
    setCMmom(px,py,pz);
  }
  ...
  void setTwists(const double x, const double y, const double z){
    l[0] = x; l[1] = y; l[2] = z;
    for(int i=0;i<3;i++){
      if(l[i] != 0.0 && l[i] != 1.0){ std::cout << "LuscherZeta::setTwists : Error, arguments must be 0 or 1\n"; exit(-1); }
    }
  }
  
  void setCMmom(const double x, const double y, const double z)
  {
    d[0] = x; d[1] = y; d[2] = z;
    double dnrm = d.norm();
    ...
    for(int i=0; i<3; i++)
    {
      if(d[i] != 0.0 && d[i] != 1.0){ std::cout << "LuscherZeta::setCMmom : Error, in comove pipi cases, CM-mom components must be 0 or 1\n"; exit(-1); }
...
//          std::cout << "\nn_vec = "<< n <<" result = "<< result <<'\n';
  ...
  inline double calcPhi(const double q, const double gamma) const{
    return (atan(-q*gamma*pow(M_PI,1.5)/calcZeta00(q,gamma)) * -180.0 / M_PI);
  }

////need further modification on this function
